...
public class check_CIDs_NonTypesystemRule extends AbstractNonTypesystemRule_Runtime implements NonTypesystemRule_Runtime {
  ...
  public void applyRule(final SNode cids, final TypeCheckingContext typeCheckingContext, IsApplicableStatus status) {
    if (isNotEmptyString(SPropertyOperations.getString(cids, "fileLocation")) && !(new File(SPropertyOperations.getString(cids, "fileLocation")).isFile())) {
      {
        MessageTarget errorTarget = new NodeMessageTarget();
        errorTarget = new PropertyMessageTarget("fileLocation");
        IErrorReporter _reporter_2309309498 = typeCheckingContext.reportTypeError(cids, "Location does not point to an existing file", "r:03143f03-46ae-4107-a067-34f5026aa223(org.campagnelab.bdval.typesystem)", "8962624141163338721", null, errorTarget);
      }
    }
  }
