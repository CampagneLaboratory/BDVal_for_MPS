...
public class Project_Behavior {
  ...
  public static void call_moveXML_1192534361192495437(SNode thisNode) {
    String projectName = SPropertyOperations.getString(thisNode, "name").replaceAll("\\s", "");
    String fileName = projectName + ".xml";
    File root = new File(MacrosFactory.getGlobal().expandPath("${org.campagnelab.bdval.home}"));
    ...
    try {
      ...
      for (Iterator iterator = allFiles.iterator(); iterator.hasNext();) {
        ...
        if (currentFile.getName().matches(fileName)) {
      ...
      File newFile = new File(SPropertyOperations.getString(SLinkOperations.getTarget(thisNode, "properties", true), "outputLocation") + "/" + projectName + "/" + fileName);
      FileUtil.copyFile(choice, newFile);
    } catch (Exception e) {
      throw new Error("Could not find/move XML file");
    }
