  ...
  public static void call_load_4345048909866934596(SNode thisNode) {
    ...
    try {
      ...
      if (datasetReader.readLine() != null) {
        throw new IllegalArgumentException("Task file must have only 1 line");
      }
      String[] lineArray = line.split("\t");
      int cols = lineArray.length;
      if ((cols % 2 == 0) || (cols < 3)) {
        throw new IllegalArgumentException("Task file must have at least 3 and an odd number of columns");
      }
      int max = (cols - 1) / 2;
      int counter = 1;
      while (counter <= max) {
      ...
      Task_Behavior.call_compareEndpts_4345048909867210476(thisNode, cids);
    } catch (Exception e) {
      throw new IllegalArgumentException("Task must be tab delimited file");
    }
  }

  public static void call_compareEndpts_4345048909867210476(SNode thisNode, SNode cids) {
